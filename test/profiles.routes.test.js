'use strict';

process.env.NODE_ENV = 'test';

const { suite, test } = require('mocha');
const request = require('supertest');
const knex = require('../knex.js');
const server = require('../index.js');
const { addDatabaseHooks } = require('./utils.js')
suite('profiles routes', addDatabaseHooks(() => {
  test('GET /profiles', (done) => {
    /* eslint-disable max-len */
    request(server)
      .get('/profiles')
      .set('Accept', 'application/json')
      .expect('Content-Type', /json/)
      .expect(200, [{
        profileId: 1,
        userId: 1,
        profileDisplayName: 'Rowling Display Name',
        description: 'I am a good roommate',
        cleanliness: 'very clean',
        guests: 'rarely',
        partyHabits: 'never',
        wakeUp: '7:00AM',
        sleep: '12:00AM',
        smoker: 'false',
        workSchedule: 'varies',
        occupation: 'student',
        spokenLanguages: JSON.stringify(['English', 'Korean', 'Chinese']),
        sex: 'Male',
        age: '26',
        relationshipStatus: 'single',
        zodiac: 'Taurus',
        monthlyBudget: '1500',
        location: 'San Francisco',
        moveInDate: '2017-11-01',
        moveOutDate: '2018-05-01',
        tags: JSON.stringify([]),
        lookingForNewRoom: 'true',
        preferredSex: 'Male',
        preferredAge: 'any',
        preferredSmoking: 'false',
        preferredLanguagesSpoken: JSON.stringify(['any']),
        createdAt: '2016-06-29T14:26:16.000Z',
        updatedAt: '2016-06-29T14:26:16.000Z'
      }], done);
    /* eslint-enable max-len */
  });

  test('GET /profiles/:id', (done) => {
    /* eslint-disable max-len */
    request(server)
      .get('/profiles/1')
      .set('Accept', 'application/json')
      .expect('Content-Type', /json/)
      .expect(200, {
        profileId: 1,
        userId: 1,
        profileDisplayName: 'Rowling Display Name',
        description: 'I am a good roommate',
        cleanliness: 'very clean',
        guests: 'rarely',
        partyHabits: 'never',
        wakeUp: '7:00AM',
        sleep: '12:00AM',
        smoker: 'false',
        workSchedule: 'varies',
        occupation: 'student',
        spokenLanguages: JSON.stringify(['English', 'Korean', 'Chinese']),
        sex: 'Male',
        age: '26',
        relationshipStatus: 'single',
        zodiac: 'Taurus',
        monthlyBudget: '1500',
        location: 'San Francisco',
        moveInDate: '2017-11-01',
        moveOutDate: '2018-05-01',
        tags: JSON.stringify([]),
        lookingForNewRoom: 'true',
        preferredSex: 'Male',
        preferredAge: 'any',
        preferredSmoking: 'false',
        preferredLanguagesSpoken: JSON.stringify(['any']),
        createdAt: '2016-06-29T14:26:16.000Z',
        updatedAt: '2016-06-29T14:26:16.000Z'
      }, done);
  });

  test('POST /profiles', (done) => {
    request(server)
      .post('/profiles')
      .set('Accept', 'application/json')
      .send({
        profile_id: 2,
        user_id: 1,
        profile_display_name: 'Rowling Display Name',
        description: 'I am a great roommate',
        cleanliness: 'very clean',
        guests: 'rarely',
        party_habits: 'never',
        wake_up: '7:00AM',
        sleep: '12:00AM',
        smoker: 'false',
        work_schedule: 'varies',
        occupation: 'student',
        spoken_languages: JSON.stringify(['English', 'Korean', 'Chinese']),
        sex: 'Male',
        age: '26',
        relationship_status: 'single',
        zodiac: 'Taurus',
        monthly_budget: '1500',
        location: 'San Francisco',
        move_in_date: '2017-11-01',
        move_out_date: '2018-05-01',
        tags: JSON.stringify([]),
        looking_for_new_room: 'true',
        preferred_sex: 'Male',
        preferred_age: 'any',
        preferred_smoking: 'false',
        preferred_languages_spoken: JSON.stringify(['any']),
        created_at: new Date('2016-06-29 14:26:16 UTC'),
        updated_at: new Date('2016-06-29 14:26:16 UTC')
      })
    .expect('Content-Type', /json/)
    .expect((res) => {
      delete res.body.createdAt;
      delete res.body.updatedAt;
    })
    .expect(200, {
      profileId: 2,
      userId: 1,
      profileDisplayName: 'Rowling Display Name',
      description: 'I am a great roommate',
      cleanliness: 'very clean',
      guests: 'rarely',
      partyHabits: 'never',
      wakeUp: '7:00AM',
      sleep: '12:00AM',
      smoker: 'false',
      workSchedule: 'varies',
      occupation: 'student',
      spokenLanguages: JSON.stringify(['English', 'Korean', 'Chinese']),
      sex: 'Male',
      age: '26',
      relationshipStatus: 'single',
      zodiac: 'Taurus',
      monthlyBudget: '1500',
      location: 'San Francisco',
      moveInDate: '2017-11-01',
      moveOutDate: '2018-05-01',
      tags: JSON.stringify([]),
      lookingForNewRoom: 'true',
      preferredSex: 'Male',
      preferredAge: 'any',
      preferredSmoking: 'false',
      preferredLanguagesSpoken: JSON.stringify(['any']),
    }, done);
      /* eslint-enable max-len */
  });

  test('DELETE /profiles/:id', (done) => {
    request(server)
      .del('/profiles/1')
      .set('Accept', 'application/json')
      .expect('Content-Type', /json/)
      .expect(200, [{
        profileId: 1,
        userId: 1,
        profileDisplayName: 'Rowling Display Name',
        description: 'I am a good roommate',
        cleanliness: 'very clean',
        guests: 'rarely',
        partyHabits: 'never',
        wakeUp: '7:00AM',
        sleep: '12:00AM',
        smoker: 'false',
        workSchedule: 'varies',
        occupation: 'student',
        spokenLanguages: JSON.stringify(['English', 'Korean', 'Chinese']),
        sex: 'Male',
        age: '26',
        relationshipStatus: 'single',
        zodiac: 'Taurus',
        monthlyBudget: '1500',
        location: 'San Francisco',
        moveInDate: '2017-11-01',
        moveOutDate: '2018-05-01',
        tags: JSON.stringify([]),
        lookingForNewRoom: 'true',
        preferredSex: 'Male',
        preferredAge: 'any',
        preferredSmoking: 'false',
        preferredLanguagesSpoken: JSON.stringify(['any']),
        createdAt: '2016-06-29T14:26:16.000Z',
        updatedAt: '2016-06-29T14:26:16.000Z'
      }], done);
  });
}));
